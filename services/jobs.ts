...
let configuredApiBaseUrl = defaultApiBaseUrl;

/**
 * Updates the base URL used for the jobs service. Helpful for native
 * environments where build-time environment variables may not be available.
 */
export const setJobsApiBaseUrl = (value: string): void => {
  configuredApiBaseUrl = value ?? '';
};
...
export interface FetchJobsOptions extends JobFilters {
  ...
  signal?: AbortSignal;
  /** Override base URL for the jobs endpoint */
  baseUrl?: string;
}
...
const createJobsUrl = (filters: JobFilters, baseUrl?: string): string => {
  ...
  const normalisedBaseUrl = resolvedBaseUrl.endsWith('/')
    ? resolvedBaseUrl.slice(0, -1)
    : resolvedBaseUrl;
  const path = `${normalisedBaseUrl}/get_jobs.php`;
...
export async function fetchJobs(options: FetchJobsOptions = {}): Promise<JobsResult> {
  const { signal, baseUrl, ...filters } = options;
  const url = createJobsUrl(filters, baseUrl);
...
export async function prefetchJobs(
  queryClient: {
    prefetchQuery: (options: {
      queryKey: QueryKey;
      queryFn: ({ signal }: { signal?: AbortSignal }) => Promise<JobsResult>;
      staleTime?: number;
    }) => Promise<unknown>;
  },
  ...
  const { signal, baseUrl, ...rest } = filters;

  const keyFilters = baseUrl ? { ...rest, baseUrl } : rest;

  await queryClient.prefetchQuery({
    queryKey: jobsQueryKeys.list(keyFilters),
    staleTime,
    queryFn: ({ signal: innerSignal }) =>
      fetchJobs({ ...rest, baseUrl, signal: innerSignal ?? signal }),
  });
}
